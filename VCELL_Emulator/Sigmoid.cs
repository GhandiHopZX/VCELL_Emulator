    ...
    public class Sigmoid
    {
        ...
        public void Train(float[][] trainingInputs, float[][] trainingOutputs)
        {
            for(var i = 0; i < trainingInputs.Length; i++)
            {
                ...
                {
                    desiredValues[desiredValues.Length - 1][j] = trainingInputs[i][j];
                }
            }
            ...
            for (var i = values.Length - 1; i >=1; i++)
            {
                ...
                {
                    biases[i][j] += HardSigmoid(values[i][j]) * learnRate;
                    biases[i][j] *= 1 - epsilon;

                    for(var k = 0; k < weights[i - 1].Length; k++)
                    {
                        weights[i - 1][j][k] += weights[i - 1][j][k] * learnRate;
